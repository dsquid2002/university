...
public class Parser {
  ...
  private void parseAnimal(String[] components) throws UnrecognizedEntryException {
    ...
    } catch (CoreDuplicateAnimalKeyException | CoreUnknownHabitatKeyException e) { // Ensure ExceptionCore1 is defined in hva.core.exception package
  ...
  private void parseVaccine(String[] components) throws UnrecognizedEntryException {
    ...
    } catch (CoreUnknownSpeciesKeyException | CoreDuplicateVaccineKeyException e) {
      throw new UnrecognizedEntryException("Invalid entry: " + e.getMessage());
    }
  }
    ...
    try {
      ...
      if (components.length == 5) {
        ...
        for (String treeKey : listOfTree){
          tree = _hotel.getTreeById(treeKey);
          _hotel.getHabitatById(id).plantTree(tree);
